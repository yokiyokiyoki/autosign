...
class LinuxDoBrowser:
    ...
    def run(self):
        ...
        logger.info(f"开始执行LinuxDo签到 - 北京时间: {beijing_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        try:
            if not self.login():
                ...
                self.send_notification_message(False, beijing_time)
            ...
            # 无论成功或失败都发送通知
            self.send_notification_message(success, beijing_time, connect_info)
    
    def send_notification_message(self, success, beijing_time, connect_info=None):
        """发送Telegram通知"""
        status = "成功" if success else "失败"
        
        # 构建基本消息
        message = f"*LinuxDo自动签到通知*\n\n" \
                 f"- 执行状态: {status}\n"
        
        # 如果有连接信息，添加到消息中
        if connect_info and len(connect_info) > 0:
            message += "\n*Connect信息*:\n"
